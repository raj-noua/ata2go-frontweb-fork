...
const InternetModal = ({ show, setShow, selectedId }) => {
    const {data, isLoading} = useGetInternetByIdQuery(selectedId)
  ...
  const handleUpdate = () => {
    updateInternet({ id: selectedId, data: {  downloadSpeed: downloadSpeed, uploadSpeed: uploadSpeed, price: price, extraInfo: extraInfo } }).then((res) => {
        if (res?.data?.status) {
