...
const ManageCars = () => {
    ...
    const handleDeleteCat = id => {
        deleteCar(id).then(res => {
            if (res?.data?.status) {
                toast.success(res?.data?.message);
                setSelectedCar(JSON.stringify({ car: "Select Car" }));
                setSelected("");
    ...
    const handleUpdate = () => {
        ...
        if (updateCar) {
            if (updateModel.length > 0) {
                updateCars({ id: selectedCar?._id, data: data }).then(res => {
                    if (res?.data?.status) {
                        toast.success(res?.data?.message);
                        setShow(false);
                        setUpdateTemp("");
    ...
    return (
        <div className="manage-video-page">
            <div className="video-upload-container">
                <div className="video-input-box">
                    ...
                    <div className="input-wrapper">
                        ...
                        <input type="text" id="carName" placeholder="Car Name" value={car} onChange={e => setCar(e.target.value)} />
                    </div>
            ...
            <div className="video-upload-container mt-4">
                <div className="video-input-box">
                    <div className="input-wrapper mb-3">
                        ...
                        >
                            <option value={JSON.stringify({ car: "Select Car" })}>Select Car</option>
                            {data?.data
                                ?.slice() // Create a shallow copy of the array
                                .sort((a, b) => a.car.localeCompare(b.car)) // Sort based on the 'system' property
                                .map(d => (
                                ...
                                {user?.role === "power" || user?.role === "admin" || user?.role === "support" ? (
                                    <div>
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                setDeleteModal(true);
                                                setSelectedItem(selectedCar?._id);
                                            }}
                                        >
                                            Delete
                                        </Button>
                                    ...
                                    <div>
                                        <Button style={{ background: "grey" }} size="md">
                                            Modify
                                        </Button>
                                        <Button className="danger-btn ms-3" variant="danger" style={{ background: "grey" }}>
                                            Delete
                                        </Button>
                                    </div>
                                )}
                ...
                <Modal.Body>
                    <div className="modal-form">
                        <div className="video-input-box-modal">
                            <h2 className="video-page-title text-center">Modify Cars</h2>
                            <div className="input-wrapper">
                                <label htmlFor="car">Car Name</label>
                                <input
                                    type="text"
                            ...
                            <div className="input-wrapper">
                                <label htmlFor="title">Add Models</label>
                                <div className="d-flex justify-content-start align-items-center">
                                    <input
                                        type="text"
                                        id="subCar"
                                    ...
                                    />
                                    <button onClick={handleAddModels} className="extra-btn">
                                        Add
                                    </button>
                                </div>
                                <div className="">
                            ...
                            <div className="input-button mt-3">
                                <Button variant="secondary me-3 danger-btn" onClick={() => setShow(false)}>
                                    Close
                                </Button>
            ...
            <DeleteModal
                question={"Do you really delete the Vehicle?"}
